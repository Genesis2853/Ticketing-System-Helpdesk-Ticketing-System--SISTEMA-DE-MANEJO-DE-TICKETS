...
import ReporteGCerrado from './ReportesGeneralesEstados/ReporteGECerrado';

const EstadoSoliCerrado = ({ user }) => {
...
useEffect(() => {
  ...
  // Búsqueda libre, normalizada y con motivo cierre incluido
  const hitLibre =
    ...
    normalize(s.motivo_cierre ?? '').includes(libre) || // motivo de cierre
    normalize(s.motivo_visita ?? "").includes(libre) ||
    normalize(s.prioridad_solicitud ?? '').includes(libre); // agrego prioridad aquí también para búsqueda libre

  const f = filtrosAvanzados;

  // Filtros avanzados normalizados, sin incluir tipoFalla ni estado
  const ok =
...
<label htmlFor="prioridad">Prioridad:</label>
                  ...
                  <div style={{ marginBottom: '20px', display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        const blob = await pdf(<ReporteGCerrado solicitudes={filteredSoliCerrado} />).toBlob();
                    ...
                    {/* Descarga */}
                    <PDFDownloadLink
                      document={<ReporteGCerrado solicitudes={filteredSoliCerrado} />}
                    ...
                    {filteredSoliCerrado.length > 0 ? (
                        filteredSoliCerrado.map(solicitud => (
                            <div key={`${solicitud.codigo_solicitud}-${solicitud.fecha_cierre}-${solicitud.codigo_trabajador}`} className="estado-solicitudes-card">
                                <h3>Solicitud: <Link to={`/estado/soliCerradoTec/${solicitud.codigo_solicitud}`}>{solicitud.codigo_ticket}</Link></h3>
                                <p>Técnico: {solicitud.nombre_tecnico} {solicitud.apellido_tecnico}</p>
...
export default EstadoSoliCerrado;
