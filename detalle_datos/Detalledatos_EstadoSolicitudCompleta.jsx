...
const DetalleSolicitudCompletoadm = () => {
    ...
    useEffect(() => {
        ...
        const fetchSolicitudAsig = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_API_URL_ESTADO}/api/estado/soliComplTec/${id}`);
                if (!response.ok) {
    ...
    const obtenerHistorial = async () => {
        ...
        try {
            const response = await axios.get(`${process.env.REACT_APP_API_URL_ESTADO}/api/estado/historial/${solicitudComplTec.codigo_solicitud}`);
    ...
    const visitasFiltradas = Array.isArray(visitas) && solicitudComplTec?.codigo_solicitud
        ...
        navigate('/descargarreportes', { state: {solicitud: solicitudComplTec} });
    };
  ...
  <div className="detalle-solicitudEstado-contenido">
    ...
    <div className="detalle-solicitudEstado-card">
      <div className="detalle-solicitudEstado-header">
        <p><span className="negrita">ID:</span> {solicitudComplTec.codigo_ticket}</p>
        <p><span className="negrita">Fecha C.:</span> {formatearFecha(solicitudComplTec.fecha_solicitud)}</p>
      </div>

      <h3 className="detalle-solicitudEstado-subtitulo">Datos Solicitud</h3>

      <div className="detalle-solicitudEstado-info">
        <p><span className="negrita">Técnico encargado:</span> {solicitudComplTec.nombre_tecnico} {solicitudComplTec.apellido_tecnico}</p>
        <p><span className="negrita">Cliente:</span> <Link to={`/cliver/vercliente/${solicitudComplTec.id_cliente}`}>{solicitudComplTec.nombre_cliente} {solicitudComplTec.apellido_cliente}</Link></p>
        <p><span className="negrita">Descripción del Servicio:</span></p>
        <p className="detalle-solicitudEstado-descripcion">{solicitudComplTec.descripcion_servicio}</p>
        <p><span className="negrita">Estado:</span> {solicitudComplTec.estado_solicitud}</p>
        <p><span className="negrita">Motivo de Visita:</span> {solicitudComplTec.motivo_visita}</p>
        <p><span className="negrita">Prioridad:</span> {solicitudComplTec.prioridad_solicitud}</p>
      </div>
    </div>
    ...
    <div className="detalle-solicitudEstado-card">
      ...
      <div className="detalle-solicitudEstado-info">
        <p><span className="negrita">Estado:</span> {solicitudComplTec.estado_solicitud}</p>
        <p><span className="negrita">Tipo Solución/Falla:</span> {solicitudComplTec.tipo_solucion_falla}</p>
        <p><span className="negrita">Herramientas Utilizadas:</span> {solicitudComplTec.herramientas_utilizadas}</p>
        <p><span className="negrita">Comentarios:</span> {solicitudComplTec.comentario_trabajo_realizado}</p>
        <p>
            <span className="negrita">Tiempo Invertido:</span>{" "}
            {formatTiempoInvertido(solicitudComplTec.tiempo_invertido)}
          </p>
        <p><span className="negrita">Fecha de cierre:</span> {solicitudComplTec.fecha_caso_cerrado}</p>
      </div>
    </div>
...
export default DetalleSolicitudCompletoadm;